How to run your own newyorkcoin Electrum server
===================================
...
requirements of about 220 MB for the Electrum database (September 2017). However note that
you also need to run newyorkcoind and keep a copy of the full blockchain,
which is roughly 1.1 GB (September 2017). Ideally you have a machine with 16 GB of RAM
and an equal amount of swap. If you have ~2 GB of RAM make sure you limit newyorkcoind 
...
only leaving spare cycles for newyorkcoind. Fast single core CPU power helps for the initial 
block chain import. Any multi-core x86 CPU with CPU Mark / PassMark > 1500 will work
(see https://www.cpubenchmark.net/). An ideal setup in September 2017 has 4 GB+ RAM and
SSD for good i/o speed.

Instructions
------------

### Step 1. Create a user for running newyorkcoind and Electrum server

This step is optional, but for better security and resource separation I
suggest you create a separate user just for running `newyorkcoind` and Electrum.
...
### Step 2. Download and compile newyorkcoind

Please see https://github.com/NewYorkCoinNYC/nycoin for detailed instructions.


### Step 3. Configure and start newyorkcoind

In order to allow Electrum to "talk" to `newyorkcoind`, we need to set up an RPC
username and password for `newyorkcoind`. We will then start `newyorkcoind` and
wait for it to complete downloading the blockchain.

    $ mkdir ~/.newyorkc
    $ $EDITOR ~/.newyorkc/newyorkcoin.conf

Write this in `newyorkcoin.conf`:
...
If you have an existing installation of newyorkcoind and have not previously
set txindex=1 you need to reindex the blockchain by running

    $ newyorkcoind -reindex

If you already have a freshly indexed copy of the blockchain with txindex start `newyorkcoind`:

    $ newyorkcoind

Allow some time to pass for `newyorkcoind` to connect to the network and start
downloading blocks. You can check its progress by running:

    $ newyorkcoind getblockchaininfo

Before starting the Electrum server your newyorkcoind should have processed all
blocks and caught up to the current height of the network (not just the headers).
You should also set up your system to automatically start newyorkcoind at boot
...
We will download the latest git snapshot for Electrum to configure and install it:
    ...
    $ cd ~
    $ git clone https://github.com/NewYorkCoinNYC/electrum-server-redux.git
    $ cd electrum-server-redux
...
leveldb should be at least version 1.9.0. Earlier versions are believed to be buggy.
...
https://github.com/NewYorkCoinNYC/electrumdb
...
Databases have grown to roughly 1.1 GB as of September 2017. Leveldb prunes the database from time to time,
...
If you're using the user `electrum` to run electrum and have added it as shown in this document, run
...
Or if you use sudo and the user is added to sudoers group:
    ...
    sudo -u electrum -i ulimit -n


Two more things for you to consider:

1. To increase privacy of transactions going through your server
   you may want to close newyorkcoind for incoming connections and connect outbound only. Most servers do run
   full nodes with open incoming connections though.

2. Consider restarting newyorkcoind (together with electrum-server) on a weekly basis to clear out unconfirmed
...
If you're operating a public newyorkcoin Electrum server please make a pull request
and add your server info to the electrum client README (at the bottom)
https://github.com/NewYorkCoinNYC/electrum-server-redux
